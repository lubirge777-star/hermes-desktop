export default {
  ...
  modeLocal: "Local",
  modeRemote: "Remote",
  modeLocalHint: "Using Lubi installed on this device",
  modeRemoteHint: "Connect to a Lubi API server on your network or cloud",
  remoteUrl: "Remote URL",
  remoteUrlHint: "The Lubi API server URL (must expose /health and /v1/chat/completions)",
  ...
  serverConfigHint: "You&apos;re connected to a remote Lubi server. Model selection, provider API keys, and credentials are managed on the server&apos;s <code>~/.hermes/.env</code> and <code>config.yaml</code>. Edit them on the host (e.g. <code>docker exec -it hermes vi /opt/data/.env</code>) and restart the container.",
  connectionMode: "Mode",
